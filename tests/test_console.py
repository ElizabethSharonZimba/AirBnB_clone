...
class TestHBNBCommand_create(unittest.TestCase):
    ...
    def test_create_object(self):
        ...
        with patch("sys.stdout", new=StringIO()) as output:
OAOAOA            self.assertFalse(HBNBCommand().onecmd("create User"))
OA            self.assertLess(0, len(output.getvalue().strip()))
OA            testKey = "User.{}".format(output.getvalue().strip())
OAOA            self.assertIn(testKey, storage.all().keys())
OAOAOAOAOA        with patch("sys.stdout", new=StringIO()) as output:
OAOA            self.assertFalse(HBNBCommand().onecmd("create State"))
OA            self.assertLess(0, len(output.getvalue().strip()))
OB            testKey = "State.{}".format(output.getvalue().strip())
OA            self.assertIn(testKey, storage.all().keys())
        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("create City"))
OAOAOAOAOA            self.assertLess(0, len(output.getvalue().strip()))
OAOAOAOA            testKey = "City.{}".format(output.getvalue().strip())
            self.assertIn(testKey, storage.all().keys())
OAOAOA        with patch("sys.stdout", new=StringIO()) as output:
            self.assertFalse(HBNBCommand().onecmd("create Amenity"))
OAOAOAOAOAOAOA            self.assertLess(0, len(output.getvalue().strip()))
            testKey = "Amenity.{}".format(output.getvalue().strip())
OAOAOAOAOA            self.assertIn(testKey, storage.all().keys())
        with patch("sys.stdout", new=StringIO()) as output:
